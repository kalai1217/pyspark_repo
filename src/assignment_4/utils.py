...
json_path = r'C:\Users\KalaiArasanJ\Desktop\PSA\pyspark_repo\resources\nested_json_file.json'
...
def create_df(spark, data, schema):
    # Create DataFrame from provided data and schema
...
# 2. Flatten the DataFrame based on custom schema
def flatten_df(df):
...
# 3. Determine record count before and after flattening
def count_before_after_flatten(df, flattened_df):
    # Calculate record count before and after flattening
    print("\nBefore Flatten: ", end="")
    print(df.count())
    print("\nAfter Flatten: ", end="")
...
# 4. Explore differences using explode, explode outer, posexplode functions
...
# 5. Filter records with empId equal to 1001
...
# 8. Create 3 new columns for year, month, and day from the load_date column
